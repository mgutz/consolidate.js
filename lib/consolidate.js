...
exports.funcd = function(path, options, fn) {
  // Funcd uses modules not scripts. `read` is uncessary.
  var engine = requires.funcd || (requires.funcd = require('funcd'));
  try {
    var html = engine.render({nocache: !options.cache}, path, options);
    fn(null, html);
  } catch (err) {
    fn(err);
  }
